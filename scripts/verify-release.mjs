...
async function gateUnifiedQuery() {
  ...
  try {
    ...
    const probe = `const {codeIntel}=await import(${JSON.stringify(path.join(ROOT, "src/engine.js"))}); const {closeIndexManager}=await import(${JSON.stringify(path.join(ROOT, "src/index-manager.js"))}); const {closeUnified}=await import(${JSON.stringify(path.join(ROOT, "src/unified.js"))}); await codeIntel({operation:"search",root:${JSON.stringify(workspace)},query:"warm the published view",freshness:"strict",limit:1,maxChars:4000,timeoutMs:120000,indexTimeoutMs:600000}); const started=Date.now(); try { const result=await codeIntel({operation:"search",root:${JSON.stringify(workspace)},query:"percentage discount invoice cents",limit:10,maxChars:8000,timeoutMs:120000,indexTimeoutMs:600000}); process.stdout.write("RESULT:"+JSON.stringify({status:result.meta.status,isError:result.isError,backends:result.meta.backends,anchored:result.meta.evidence.filter((e)=>e.anchor?.span?.coordinateSystem === "utf8-bytes" && e.sourceCheck === "matched").length,queryMs:Date.now()-started})+"\\n"); } finally { closeIndexManager(); await closeUnified(); }`;
