...
function queryProbe(root, events) {
  return [
    ...
    "try {",
    "  phase('publish');",
    "  await codeIntel({ operation: 'search', root, query: 'warm the published view', freshness: 'strict', limit: 1, maxChars: 4000, timeoutMs: 120000, indexTimeoutMs: 300000 });",
    "  await codeIntel({ operation: 'architecture', root, query: 'start', symbol: 'start', relativePath: 'flow.mjs', freshness: 'strict', limit: 1, maxChars: 4000, timeoutMs: 120000, indexTimeoutMs: 300000 });",
    "  phase('retrieval');",
