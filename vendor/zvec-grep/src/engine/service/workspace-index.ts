...
import {
  ...
  indexWorkspace,
  indexWorkspacePaths,
  indexWorkspacePrepared,
  type PreparedSnapshotBatch,
  type PreparedSnapshotResult,
...
export class WorkspaceIndex {
  ...
  indexPrepared(batch: PreparedSnapshotBatch): Promise<PreparedSnapshotResult> {
    if (this.storage.readOnly) {
      throw new EngineError("Cannot update a read-only workspace index", {
        code: "ZVEC_GREP.ENGINE.WORKSPACE_INDEX.READ_ONLY",
        context: workspaceIndexOperationDetails(this.name, "indexPrepared"),
      });
    }
    const embeddingModel = this.requireEmbeddingModel("indexPrepared");
    return indexWorkspacePrepared(
      {
        workspaceIndex: this.info,
        embeddingModel,
        storage: this.storage,
