...
import { statsForSnapshot, type SnapshotInput } from './snapshot-input';
/**
...
export type { SnapshotInput } from './snapshot-input';
/**
...
export class ExtractionOrchestrator {
   ...
   * Index a single file
   */
  /** Index one captured source without reading its path. */
  async indexSnapshot(snapshot: SnapshotInput): Promise<ExtractionResult> {
    await initGrammars();
    const language = detectLanguage(snapshot.relativePath, snapshot.content, loadExtensionOverrides(this.rootDir));
    await loadGrammarsForLanguages([language]);
    return this.indexFileWithContent(snapshot.relativePath, snapshot.content, statsForSnapshot(snapshot));
  }

  /** Apply captured upserts/deletions through the normal incremental store path. */
  async syncSnapshots(
    ...
    for (const filePath of deletedPaths) {
      ...
      const resurrected = incoming
        ...
        .filter((reference): reference is UnresolvedReference => reference !== null);
    ...
    for (const snapshot of snapshots) {
      ...
      const result = await this.indexFileWithContent(
        ...
        snapshot.content,
        statsForSnapshot(snapshot),
      );
      nodesUpdated += result.nodes.length;
    }

    const afterPairs = new Set(this.queries.getNodeNamePairsByFiles(changedPaths));
    const definitionNames = new Set<string>();
    const nameOf = (pair: string) => pair.slice(pair.indexOf('\0') + 1);
    for (const pair of beforePairs) if (!afterPairs.has(pair)) definitionNames.add(nameOf(pair));
    for (const pair of afterPairs) if (!beforePairs.has(pair)) definitionNames.add(nameOf(pair));
    return {
      filesChecked: changedPaths.length,
      filesAdded,
      filesModified,
      filesRemoved,
      nodesUpdated,
      durationMs: Date.now() - startTime,
      changedFilePaths: changedPaths.length > 0 ? changedPaths : undefined,
      definitionDelta: definitionNames.size > 0 ? [...definitionNames] : undefined,
    };
  }
