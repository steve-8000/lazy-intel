...
export interface LazyCallPath {
  readonly nodeIds: readonly string[];
  readonly synthesizedHops: readonly {
...
export interface LazyTaskContextData {
  readonly context: TaskContext;
  readonly callPaths: readonly LazyCallPath[];
  readonly confidence: Subgraph['confidence'];
}
...
export class ContextBuilder {
  ...
  async buildContext(
    ...
    const data = await this.buildContextData(input, options);
    ...
    if (opts.format === 'markdown') {
      ...
      const isGenerated = this.queries.generatedPredicateFor([
        ...data.context.entryPoints.map((n) => n.filePath),
        ...Array.from(data.context.subgraph.nodes.values(), (n) => n.filePath),
        ...data.context.codeBlocks.map((b) => b.filePath),
      ]);
      return formatContextAsMarkdown(data.context, isGenerated)
        + this.buildCallPathsSection(data.context.subgraph, data.callPaths)
        + (data.confidence === 'low' ? this.buildLowConfidenceNote(data.context.entryPoints) : '');
    } else if (opts.format === 'json') {
      return formatContextAsJson(data.context);
    }
    return data.context;
  }

  /** Build structured context and typed call paths without formatting. */
  async buildContextData(
    input: TaskInput,
    options: BuildContextOptions = {}
  ): Promise<LazyTaskContextData> {
    const opts = { ...DEFAULT_BUILD_OPTIONS, ...options };
    ...
    const context: TaskContext = {
      query, subgraph, entryPoints, codeBlocks, relatedFiles, summary, stats,
    };
    return { context, callPaths: this.selectCallPaths(subgraph), confidence: subgraph.confidence };
  }
   ...
   */
  private selectCallPaths(subgraph: Subgraph): LazyCallPath[] {
    ...
    for (const e of subgraph.edges) {
      ...
      const label = m.synthesizedBy === 'callback'
        ? 'callback via ' + (m.via ? tick + String(m.via) + tick : 'registrar') + at
        ...
        : 'event ' + (m.event ? tick + String(m.event) + tick : '') + at;
      synthByPair.set(e.source + '>' + e.target, label);
    }
    return kept.map((nodeIds) => ({
      nodeIds,
      synthesizedHops: nodeIds.slice(1).flatMap((target, i) => {
        const source = nodeIds[i]!;
        const label = synthByPair.get(source + '>' + target);
        return label ? [{ source, target, label }] : [];
      }),
    }));
  }

  private buildCallPathsSection(subgraph: Subgraph, callPaths: readonly LazyCallPath[]): string {
    if (callPaths.length === 0) return '';
    const name = (id: string): string => subgraph.nodes.get(id)?.name ?? id;
    const renderChain = (callPath: LazyCallPath): string => {
      const synthByPair = new Map(callPath.synthesizedHops.map((hop) => [hop.source + '>' + hop.target, hop.label]));
      let s = name(callPath.nodeIds[0]!);
      for (let i = 1; i < callPath.nodeIds.length; i++) {
        const source = callPath.nodeIds[i - 1]!;
        const target = callPath.nodeIds[i]!;
        const synth = synthByPair.get(source + '>' + target);
        s += synth ? ' →[' + synth + '] ' + name(target) : ' → ' + name(target);
      }
      return s;
    };
    const hasSynth = callPaths.some((callPath) => callPath.synthesizedHops.length > 0);
    const tick = String.fromCharCode(96);
    const lines = [
      '', '## Call paths', '',
      'Execution flow among the key symbols (traced through the call graph):', '',
      ...callPaths.map((callPath) => '- ' + renderChain(callPath)), '',
      hasSynth
        ? '_Hops marked ' + tick + '[callback/event …]' + tick + ' are dynamic dispatch bridged by codegraph (with the registration site); the rest are direct calls. codegraph_node any symbol for its body._'
