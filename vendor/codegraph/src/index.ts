...
import type { LazyTaskContextData } from './context';
...
// MODIFIED BY lazy-intel (see vendor/codegraph/UPSTREAM.json local_patches).
// Upstream re-exported `MCPServer` from './mcp' here. That single line pulled the
// whole server subtree onto the library's cold-start path, and with it
// '../upgrade/update-check' (a background GitHub update probe, src/mcp/index.ts:53)
// and '../telemetry'. Neither may be reachable from a query. lazy-intel embeds the
// library and never constructs MCPServer; the only in-tree consumer, src/bin/codegraph.ts:1867,
// already loads it with a dynamic `import('../mcp/index')` and is unaffected.
...
export class CodeGraph {
  ...
  /** Build typed context data without formatting it for a text consumer. */
  async buildContextData(
    input: TaskInput,
    options?: BuildContextOptions
  ): Promise<LazyTaskContextData> {
    return this.contextBuilder.buildContextData(input, options);
  }
